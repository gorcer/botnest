...
export class CloseOrderService {
  ...
  public async create(orderInfo: RequestSellInfoDto): Promise<Order> {
    ...
    if (extOrder && extOrder.id != undefined) {
      ...
      await this.orders.update(orderInfo.id, {
        preclosed: add(orderInfo.preclosed, extOrder.amount),
      });
  ...
  public async rollback(order: Order) {
    ...
    await this.orders.update(parentOrder.id, {
      preclosed: subtract(parentOrder.preclosed, order.amount1),
    });
  ...
  public async check(order: Order, extOrder?): Promise<Order> {
    ...
    if (compareTo(extOrder.filled, extOrder.amount) == 0) {
      ...
      const updateOrderDto: UpdateOrderDto = {
        closed_amount: add(parentOrder.closed_amount, extOrder.filled),
      };
      if (compareTo(parentOrder.amount1, updateOrderDto.closed_amount) == 0) {
